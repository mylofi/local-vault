...
const LV_SRC = path.join(SRC_DIR,"lv.js");
const NODE_MODULES_DIR = path.join(PKG_ROOT_DIR,"node_modules");
const LOFI_LDL_DIST_DIR = path.join(NODE_MODULES_DIR,"@lo-fi","local-data-lock","dist");
const LOFI_LDL_DIST_AUTO_DIR = path.join(LOFI_LDL_DIST_DIR,"auto");
const LOFI_LDL_DIST_BUNDLERS_DIR = path.join(LOFI_LDL_DIST_DIR,"bundlers");
...
const DIST_AUTO_EXTERNAL_LOFI_LDL_DIR = path.join(DIST_AUTO_EXTERNAL_LOFI_DIR,"local-data-lock");
...
async function main() {
	...
	for (let dir of [
			...
			DIST_AUTO_EXTERNAL_LOFI_LDL_DIR,
	...
	await buildFiles(
		recursiveReadDir(LOFI_LDL_DIST_AUTO_DIR),
		LOFI_LDL_DIST_AUTO_DIR,
		DIST_AUTO_EXTERNAL_LOFI_LDL_DIR,
		// simple copy as-is
